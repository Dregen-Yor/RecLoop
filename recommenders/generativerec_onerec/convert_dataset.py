...
def load_dataset(data_dir: str, dataset_name: str, CID: int) -> Dict[str, Any]:
    """Load all dataset files"""
    data = {}
    
    if CID == 1:
        index_file = f'{dataset_name}-sasrec.index.json'
    else:
        index_file = f'{dataset_name}.index.json'
    item_file = f'{dataset_name}.item.json'
    # Load item metadata (id -> {title, description, ...})
    with open(os.path.join(data_dir, item_file), 'r') as f:
        data['items'] = json.load(f)
    
    # Load item_id to semantic tokens mapping from index.json
    with open(os.path.join(data_dir, index_file), 'r') as f:
...
def convert_interactions_to_csv(splits: Dict[str, List], items: Dict[str, Dict], 
                               ...
                               keep_longest_only: bool = True, CID: int = 0):
    ...
    for split_name, split_data in splits.items():
        ...
        if rows:
            ...
            if CID == 1:
                output_file = os.path.join(output_dir, f'{category}_5_2016-10-2018-11-CID.csv')
            else:
...
def main():
    ...
    parser.add_argument('--CID', type=int, default=0,
                       help='CID for output file naming, 1 for collaborative dataset, 0 for semantic dataset')
    ...
    dataset_data = load_dataset(args.data_dir, args.dataset_name, args.CID)
    ...
    if args.CID == 1:
        info_file = os.path.join(args.output_dir, 'info', f'{args.category}_5_2016-10-2018-11-CID.txt')
    else:
