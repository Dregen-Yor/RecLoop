...
def train(config, device, item_embedding, id_split, id_save_location):
    ...
    if os.path.exists(id_save_location):
        ...
        per_ds = config["dataset"].get("per_dataset_rqvae", {})
        if dataset_name in per_ds:
            ds_override = per_ds[dataset_name]
            if "input_dim" in ds_override:
                model_config["input_dim"] = ds_override["input_dim"]
            if "hidden_dim" in ds_override:
                model_config["hidden_dim"] = list(ds_override["hidden_dim"])
            print(f"[RQ-VAE] Dataset '{dataset_name}' override: input_dim={model_config['input_dim']}, hidden_dim={model_config['hidden_dim']}")
