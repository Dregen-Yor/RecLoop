...
def expand_id_arr(id_arr, codebook_size):
    ...
    if id_arr.size == 0 or len(id_arr.shape) < 2:
        return
...
def load_data(
    ...
    if len(unseen_val) > 0:
        ...
        val_unseen_semantic_ids = np.empty((0, n_codebook), dtype=np.int64)
    ...
    if len(unseen_test) > 0:
        ...
        test_unseen_semantic_ids = np.empty((0, n_codebook), dtype=np.int64)
