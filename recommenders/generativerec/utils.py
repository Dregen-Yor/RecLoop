...
_GENERATIVEREC_DIR = os.path.dirname(os.path.abspath(__file__))


def resolve_generativerec_path(path: str) -> str:
    """Resolve a path relative to recommenders/generativerec/ when not absolute."""
    if os.path.isabs(path):
        return os.path.normpath(path)
    return os.path.normpath(os.path.join(_GENERATIVEREC_DIR, path))


def cf_embedding_path(cf_model: str, dataset_name: str) -> str:
    """Path to collaborative-filtering item embeddings from tranditionalrec."""
    return resolve_generativerec_path(
        os.path.join(
            "..",
            "tranditionalrec",
            "output",
            f"{cf_model}-{dataset_name}_item_embeddings.pt",
        )
    )
